<script lang="ts">
  ...
  const kpis = (d: DashboardData) => [
    { label: 'Ingresos hoy',  value: fmt(d.kpis.ingresosHoy),     color: 'green',
      icon: 'bi-arrow-down-circle' },
    { label: 'Egresos hoy',   value: fmt(d.kpis.egresosHoy),      color: 'red',
      icon: 'bi-arrow-up-circle' },
...
<div class="p-3 p-md-4">
  ...
  <div class="quick-actions mb-4">
    {#each visibleQuickActions as qa}
      <a href={qa.href} class="quick-actions__item">
        <div class="quick-actions__icon{qa.color ? ` quick-actions__icon--${qa.color}` : ''}">
          <i class="bi {qa.icon}"></i>
        </div>
        <div class="quick-actions__label">{qa.label}</div>
      </a>
    {/each}
  </div>
