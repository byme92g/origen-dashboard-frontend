              ...
              {#each items as c}
                <tr>
                  ...
                  <td class="pe-3 text-end">
                    <button class="btn btn-xs btn-outline-secondary btn-sm me-1" on:click={() => openEdit(c)}>✏️</button>
                    {#if $isAdmin}
                      <button class="btn btn-xs btn-outline-danger btn-sm" on:click={() => confirmDelete(c.id)}>🗑️</button>
                    {/if}
                  </td>
                </tr>
              {:else}
